...
const fechaParaInput = (fecha: Date) => `${fecha.getFullYear()}-${String(fecha.getMonth() + 1).padStart(2, '0')}-${String(fecha.getDate()).padStart(2, '0')}`;
...
export function CitasRecepcionView() {
  ...
  const [horarioElegido, setHorarioElegido] = useState<{ fecha: string; hora: string } | null>(null);
  ...
  if (modoAgendar)
    return (
      <AgendarConsultaView
        ...
        consultaPendiente={citaPorCobrar ? { paciente: citaPorCobrar.paciente, ci: citaPorCobrar.ci, doctor: citaPorCobrar.doctor, especialidad: citaPorCobrar.motivo } : undefined}
        onVolver={() => { setModoAgendar(false); setCitaPorCobrar(null); }}
        onAgendada={(datos) => {
          const fechaAgendada = new Date(`${datos.fecha}T00:00:00`);
          const inicioVisible = new Date(semana.getFullYear(), semana.getMonth(), semana.getDate());
          const diaAgendado = Math.round((fechaAgendada.getTime() - inicioVisible.getTime()) / 86400000);
          setCitasProgramadas((actuales) => citaPorCobrar
            ? actuales.map((item) => item === citaPorCobrar ? { ...item, estado: 'Confirmada' } : item)
            : [...actuales, { dia: Math.min(Math.max(diaAgendado, 0), 5), hora: datos.hora, paciente: datos.paciente, ci: datos.ci, doctor: datos.doctor, motivo: 'Medicina general', estado: 'Confirmada' }]);
  ...
  return (
    <section className="citas-vista">
      <section className="panel citas-controles">
        ...
        <div className="citas-doctor">
          ...
          <button
            ...
            type="button"
            onClick={() => { setHorarioElegido(null); setModoAgendar(true); }}
          >
            ...
            <div className="agenda-fila" key={hora}>
              ...
              {dias.map((_, dia) => {
                const cita = citasVisibles.find(
                  (item) => item.dia === dia && item.hora === hora && item.estado !== 'Cancelada',
                );
                return (
                  <div
                    ...
                    aria-label={cita ? undefined : `Agendar consulta el ${dias[dia]} a las ${hora}`}
                    onClick={() => { if (!cita) { setHorarioElegido({ fecha: fechaParaInput(sumarDias(semana, dia)), hora }); setModoAgendar(true); } }}
                    onKeyDown={(event) => { if (!cita && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); setHorarioElegido({ fecha: fechaParaInput(sumarDias(semana, dia)), hora }); setModoAgendar(true); } }}
                  >
                    {cita && (
                      <article
                        ...
                        role="button"
                        tabIndex={0}
                        onClick={(event) => { event.stopPropagation(); setCitaInspeccionada(cita); }}
                        onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setCitaInspeccionada(cita); } }}
                      >
      ...
      {citaInspeccionada && (
        <div className="citas-modal-fondo" onClick={() => { setCitaInspeccionada(null); setConfirmarCancelacion(false); }}>
          <section className="cita-detalle-modal" onClick={(event) => event.stopPropagation()}>
            <header><div><span>DETALLE DE CONSULTA</span><h2>{citaInspeccionada.paciente}</h2></div><button type="button" onClick={() => setCitaInspeccionada(null)} aria-label="Cerrar"><Icon name="close" size={18} /></button></header>
            <dl><div><dt>CI</dt><dd>{citaInspeccionada.ci}</dd></div><div><dt>Estado</dt><dd><em className={citaInspeccionada.estado.toLowerCase().replace(' ', '-')}>{citaInspeccionada.estado}</em></dd></div><div><dt>Doctor</dt><dd>{citaInspeccionada.doctor}</dd></div><div><dt>Fecha y hora</dt><dd>{dias[citaInspeccionada.dia]} · {citaInspeccionada.hora}</dd></div><div><dt>Especialidad</dt><dd>{citaInspeccionada.motivo}</dd></div><div><dt>Pago</dt><dd>{citaInspeccionada.estado === 'Confirmada' ? 'Pagado' : 'Pendiente'}</dd></div></dl>
            {confirmarCancelacion ? <div className="cita-cancelar-aviso"><Icon name="audit" size={22} /><div><strong>¿Seguro que deseas cancelar esta consulta?</strong><p>El horario quedará libre para otro paciente. Esta acción conservará el registro como cancelado.</p></div><footer><button className="secundario" type="button" onClick={() => setConfirmarCancelacion(false)}>No, volver</button><button className="cita-confirmar-cancelacion" type="button" onClick={() => { setCitasProgramadas((actuales) => actuales.map((item) => item === citaInspeccionada ? { ...item, estado: 'Cancelada' } : item)); setCitaInspeccionada(null); setConfirmarCancelacion(false); }}>Sí, cancelar consulta</button></footer></div> : citaInspeccionada.estado === 'Por confirmar' && <footer className="cita-detalle-acciones"><button className="cita-cancelar" type="button" onClick={() => setConfirmarCancelacion(true)}>Cancelar consulta</button><button className="primario" type="button" onClick={() => { setCitaPorCobrar(citaInspeccionada); setHorarioElegido({ fecha: fechaParaInput(sumarDias(semana, citaInspeccionada.dia)), hora: citaInspeccionada.hora }); setCitaInspeccionada(null); setModoAgendar(true); }}><Icon name="check" size={15} /> Confirmar y cobrar</button></footer>}
          </section>
        </div>
      )}
      {agendarAbierto && (
